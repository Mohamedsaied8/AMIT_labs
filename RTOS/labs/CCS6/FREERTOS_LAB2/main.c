...
uint8_t speed = 1; /*shared global variable !!, may cause problems (not our scope in this lab)
...
void vTaskState(void *para)
{
 ...
 int main(void) {
	...
	xTaskCreate(vLEDTask, "LED",TASKSTACKSIZE, NULL,1, &First_handle);
	xTaskCreate(vButtonTask, "Button",TASKSTACKSIZE, NULL,2, &Second_handle);
	handles.first_task = First_handle;
	handles.second_task = Second_handle;
	xTaskCreate(vTaskState, "Statistics",TASKSTACKSIZE, (void * )(&handles),3, &Third_handle);
